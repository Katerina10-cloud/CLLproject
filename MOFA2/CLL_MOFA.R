...
#for TFT
resTFT <- filter(testTab, !is.na(TFT)) %>%
  group_by(factor) %>%
  do(comSurv(.$value, .$TFT, .$treatment, TRUE)) %>% ungroup() %>%
  arrange(p) %>% mutate(p.adj = p.adjust(p, method = "bonferroni")) %>%
  mutate(Endpoint = "TFT")

#Plot p values and hazard ratios
plotTab <- bind_rows(resOS, resTTT, resTFT) %>%
  filter(factor %in% c("Factor1","Factor2","Factor3", "Factor4", "Factor5", "Factor6", "Factor7"))
...
facList <- sort(filter(resOS, p<=0.4)$factor)
...
facList <- sort(filter(resTTT, p<=0.05)$factor)
...
tftList <- lapply(facList, function(x) {
  eachTab <- filter(testTab, factor == x) %>%
    select(value, TFT, treatment) %>% filter(!is.na(TFT))
  pval <- filter(resTFT, factor == x)$p
  km(eachTab$value, eachTab$TFT, eachTab$treatment, sprintf("%s VS TFT", x), stat = "maxstat",
     maxTime = 7, pval = pval, showTable = TRUE)
})
grid.arrange(grobs = tftList, ncol = 3)

#KM plot for subgroup defined by IGHV status and median latent factor values
groupTab <- filter(testTab, factor == "Factor1", !is.na(value), !is.na(IGHV)) %>%
  mutate(subgroup = ifelse(value > median(value), paste0(IGHV,"_highFactor1"), paste0(IGHV,"_lowFactor1"))) 
...
resTFT <- runCox(survTab, riskTab, "TFT", "treatment")
#summary(surv1)
haTFT <- plotHazard(resTFT, title = "TFT") +
  scale_y_log10(limits=c(0.2,5))
haTFT

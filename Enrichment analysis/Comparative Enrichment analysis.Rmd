...
gc_Reactome_2.results.df <- gc_gseReactomePA@compareClusterResult
```
```{r}
#subset columns
gc_Reactome_2.results.df <- gc_Reactome_2.results.df[c("Cluster", "ID", "Description", "pvalue", "qvalue", "core_enrichment")]
```
```{r}
#format core_enrichment column
gc_Reactome_2.results.df$core_enrichment <- gsub("/", ",",gc_Reactome_2.results.df$core_enrichment)
```
```{r}
# change column headers
colnames(gc_Reactome_2.results.df) <- c("phenotype","Name", "Description","pvalue","qvalue", "genes")
```
```{r}
gc_Reactome_2.results.df <- gc_Reactome_2.results.df %>% relocate(phenotype, .before = genes)
```
```{r}
gc_Reactome_2.results.df$phenotype <- as.character(gc_Reactome_2.results.df$phenotype) 
```
```{r}
gc_Reactome_2.results.df["phenotype"][gc_Reactome_2.results.df["phenotype"] == "F4_up"] <- "1"
```
```{r}
gc_oraGO_2.results.df$phenotype <- as.numeric(gc_oraGO_2.results.df$phenotype) 
```
```{r}
gc_Reactome_2.results.filename <- file.path(getwd(),paste("clusterprofiler_cluster_enr_results.txt",sep="_"))
write.table(gc_Reactome_2.results.df,gc_Reactome_2.results.filename,col.name=TRUE,sep="\t",row.names=FALSE,quote=FALSE)
```
```{r}
...
em_command = paste('enrichmentmap build analysisType="generic" ', 
                   ...
                   'enrichmentsDataset1=', gc_Reactome_2.results.filename,
                   sep=" ")
```
```{r}

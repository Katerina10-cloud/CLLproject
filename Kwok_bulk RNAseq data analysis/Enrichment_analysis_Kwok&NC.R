...
#Preparing input
geneList <- as.vector(res2$log2FoldChange)
...
(enrNetwork1, categorySize="pvalue", foldChange=geneList1, cex_label_gene = 0.6, cex_label_category = 0.7, color_category='firebrick')+ ggtitle("Over-expression analysis Hallmark Indolent vs Progressive")
p1
...
#MSigDb gene set enrichment analysis
msig_H1_gsea <- GSEA(geneList, TERM2GENE = msig_H)
head(msig_H1_gsea)
...
p1 <- cnetplot(enrNetwork1, foldChange=geneList, cex_label_gene = 0.7, cex_label_category = 0.8, color_category='firebrick', showCategory = "HALLMARK_OXIDATIVE_PHOSPHORYLATION")
p1
dotplot(msig_H1_gsea, showCategory=10, font.size = 10, title = "Hallmark Regressive vs Progressive")
heatplot(enrNetwork1, foldChange=geneList, showCategory=6)
...
geneList <- as.vector(corRes.sig_F6$logFC)
...
msigH_ora_F6 <- enricher(gene, TERM2GENE=msig_H)
head(msigH_ora_F1)

dotplot(msigH_ora_F6, showCategory=10, label_format = 30, font.size = 10, title = "Hallmarks ORA Factor6")
...
#MSigDb GSEA
msigH_gsea_F6 <- GSEA(geneList, TERM2GENE = msig_H)
head(msigH_gsea_F6)

dotplot(msigH_gsea_F6, showCategory=10, label_format = 40, font.size = 12, title = "Hallmarks GSEA F6")

enrNetwork <- setReadable(msigH_gsea_F6, 'org.Hs.eg.db', 'ENTREZID')
p1 <- cnetplot(enrNetwork, categorySize="pvalue", foldChange=geneList, showCategory=3, cex_label_gene = 0.6, cex_label_category = 0.8, color_category='firebrick', node_label="category")+ ggtitle("Hallmarks GSEA F6")
p1


# convert gene ID to Symbol
enrNetwork <- setReadable(msigH_gsea_F6, 'org.Hs.eg.db', 'ENTREZID')
cnetplot(enrNetwork, categorySize="pvalue", foldChange=geneList, showCategory = 3, cex_label_gene = 0.6, cex_label_category = 0.8, color_category='firebrick') + ggtitle("Hallmarks GSEA F6")
cnetplot(enrNetwork, foldChange=geneList, circular = TRUE, colorEdge = TRUE, cex_label_gene = 0.6, cex_label_category = 0.8, color_category='firebrick')
heatplot(enrNetwork, foldChange=geneList, showCategory=3)
...
corResF6_down <- subset(corRes, corRes$adj.P.Val < 0.05 & corRes$logFC < 0)
...
#ReactomePA GSEA
Ind_down <- geneList1_down
R_down <- geneList2_down
R2_down <- geneList3_down
X3_down <- geneList3_down
F4_up <- geneListF4_up
F6_down <- geneListF6_down

gc_gsea <- list(Ind_down=Ind_down, R_down=R_down, F6_down=F6_down, F4_up=F4_up)
gc_gseReactomePA <- compareCluster(geneCluster = gc_gsea, fun = gsePathway, minGSSize = 10, maxGSSize = 350, pvalueCutoff = 0.01)
gc_gseReactomePA <- setReadable(gc_gseReactomePA, OrgDb = org.Hs.eg.db, keyType="ENTREZID")
dotplot(gc_gseReactomePA, showCategory = 20, label_format = 80, font.size = 9, title = "Integrative enrichment comparison ReactomePA")
...
gc_oraGO_2.results.df$term.size <- gsub("/(\\d+)", "", gc_oraGO_2.results.df$BgRatio)

## filter for term size to keep only term.size => 3, gene count >= 5 and subset
gc_oraGO_2.results.df <- gc_oraGO_2.results.df[which(gc_oraGO_2.results.df[,'term.size'] >= 2 & gc_oraGO_2.results.df[,'Count'] >= 5),]
